...
// Displayed version. The '++' marks this fork, placed after the number exactly as
// it appears in the .jsx filename. Internal comparisons keep using scriptVersion_XX.
var scriptVersion_PP 		= scriptVersion_XX + '++';
var scriptVersion_vXX 		= 'v' + scriptVersion_XX;

var scriptLastEdit_FR		= " -- 01/09/2026";
var scriptLastEdit_LOC		= " -- 2026/09/01";
...
message_fr["UI::Title"] 					= "Import TVPaint 12 -- v."+scriptVersion_PP+scriptLastEdit_FR;
...
message_en["UI::Title"] 					= "Import TVPaint 12 -- v. "+scriptVersion_PP+scriptLastEdit_LOC;
...
message_ja["UI::Title"] 					= "TVPaint 12 -- v. "+scriptVersion_PP+"を読み込みする"+scriptLastEdit_LOC;
...
message_zh["UI::Title"] 					= "导入 TVPaint 12 -- v. "+scriptVersion_PP+scriptLastEdit_LOC;
...
function OpenShotBrowserDialog(initialBaseFolder, settings) {
    ...
    var win = new Window("dialog", (message[lang]["UI::Browser::Title"] || "TVPaint Shot Browser") + " -- v." + scriptVersion_PP);
...
function ShowWarningReport( iSettings ) {
    ...
    // A failed shot has no layer to mark, so its composition and project folder are
    // marked instead -- those are what has to be re-imported.
    function ApplyFailureFlags( iOn ) {
        app.beginUndoGroup( Msg("UI::Report::FlagUndo", "TVPaint Import -- Flag Affected Layers") );
        try {
    ...
    // Belongs with the picker above it: whatever is left unfixed can be marked for
    // later. Off by default -- the label it overwrites carries the TVPaint group colour.
    var flagGroup = win.add("group");
    flagGroup.orientation = "row";
    flagGroup.alignChildren = ["left", "center"];
    var chkFlagLayers = flagGroup.add("checkbox", undefined,
        Msg("UI::Report::Flagged", "Label unresolved layers Red (still set to Normal)"));
    chkFlagLayers.value = LoadBoolSetting("FlagWarnings", false);
    chkFlagLayers.onClick = function() {
    ...
    if( chkFlagLayers.value ) {
        ApplyLayerFlags( true );
    }
